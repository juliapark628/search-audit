...
legal_help_org_rankings <- 
  ...
  filter(is_legal_org == TRUE) %>% 
  filter(full_url %in% legal_help_orgs) %>% 
...
legal_help_org %>% 
  write_csv(str_glue("processed_data/", which_audit, "/legal_help_serp_complete.csv"))

```


```{r}
legal_help_org_perc_state <- 
  all_tables %>% 
  mutate(
    is_legal_org = case_when(
      urlsuffix == "gov" ~ TRUE, 
...
legal_help_orgs <- 
  legal_help_org_perc_state$full_url

legal_help_org_rankings_state <- 
  all_tables %>% 
  mutate(
    is_legal_org = case_when(
      urlsuffix == "gov" ~ TRUE, 
...
legal_help_org_state <- 
  legal_help_org_perc_state %>% 
  left_join(legal_help_org_rankings_state, by = c("state", "legal_type", "full_url")) %>% 
